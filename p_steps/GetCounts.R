...
for(j in t.interest){
  ...
  if(j %in% dbListTables(mydb)){
    ...
    #[,  N_masked :=  fifelse(N < 5, "< 5", as.character(N)) ][, N := NULL]
    saveRDS(as.data.table(VALUES),paste0(projectFolder, "/g_intermediate/WHERECLAUSE_",j,".rds"))
    rm(VALUES)
    gc()
    ...
    saveRDS(VALUES3,paste0(projectFolder, "/g_intermediate/AWNSERS_",j,".rds")) 
    rm(VALUES3)
    }
