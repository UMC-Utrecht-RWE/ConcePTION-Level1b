...
cols <- colnames(file)[!colnames(file)  %in% c("N", "Study_variable", "sens", "count", "meanN", "col", "id")]
cols2 <-  as.data.table(expand.grid(cols, cols, stringsAsFactors = F))[Var1 != Var2,]
...
i <- 15
          for(i in 1:length(cols2)){
            ...
            col_tmp <- cols[!cols %in% c(cols2[[i]], "N", "Study_variable", "sens", "count", "meanN", "col", "id")]
            ...
            file <- file[sens > cutoff, `:=` (count = count + 1, col = paste0(cols2[[i]], collapse = "|"), id = idtmp)][, idtmp := NULL]
